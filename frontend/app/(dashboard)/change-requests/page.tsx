...
export default function StaffChangeRequests() {
  ...
  const load = () => api.get("/staff/change-requests").then((r) => setItems(r.data)).catch((e) => setError(apiError(e)));
  useEffect(() => { load(); }, []);

  if (user && !user.can_cr) {
    return <div className="glass p-6 text-muted">You are not assigned to Change Requests.</div>;
  }

  async function approve(id: number) {
    setMsg(""); setError("");
    if (!window.confirm("Approve and apply this change to the member?")) return;
    try { const { data } = await api.post(`/staff/change-requests/${id}/approve`); setMsg(data.message); load(); }
  ...
  async function reject(id: number) {
    ...
    if (note === null) return;
    setMsg(""); setError("");
    try { const { data } = await api.post(`/staff/change-requests/${id}/reject`, { note }); setMsg(data.message); load(); }
    catch (e) { setError(apiError(e)); }
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-sm text-muted">Member email / wallet address changes awaiting approval (already confirmed by email TAC).</p>
