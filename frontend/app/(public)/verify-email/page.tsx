...
function VerifyForm() {
  ...
  async function resend() {
    ...
    try { const { data } = await api.post("/resend-code", { email }); setMsg(data.message); }
  ...
  return (
    <div className="w-full max-w-md mx-auto px-5">
      ...
      <div className="glass p-8 animate-fade-up">
        <div className="flex items-center gap-2"><MailCheck className="text-gold-light" /><h1 className="text-2xl font-bold">Verify your email</h1></div>
        <p className="text-muted text-sm mt-1">We sent a 6-digit code to <b className="text-foreground">{email || "your email"}</b>. Enter it below to activate your account.</p>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <button disabled={loading || code.length < 6} className="btn-gold w-full py-2.5">{loading ? "Verifying…" : "Verify & Continue"}</button>
        </form>
        ...
        <p className="mt-4 text-xs text-muted">Didn&apos;t get it? Check your Spam/Promotions folder.</p>
      </div>
    </div>
  );
}
